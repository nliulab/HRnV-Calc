...
HRnV is a method applied on RRI. Therefore, the extraction of RRI from ECG for HRnV is exactly the same as conventional HRV. Once the original RRI obtained, HRnV will generate new intervals called RR<sub>n</sub>I<sub>m</sub>, which is similar to RRI. To fully describe the process of RR<sub>n</sub>I<sub>m</sub>, consider a series clean RRI (i.e., all outliers and non-sinus beats are removed or processed) ![equation](https://bit.ly/38PYNW3)
