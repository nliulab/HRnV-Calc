function [cleanNN, cleantNN, flagged_beats] = RRIntervalPreprocess_Revised(rr,t_rr,HRVparams)
%   Revised by Duke-NUS Team
...
rr(idx_remove+1) = [];
t_rr(idx_remove) = [];
%annotations(idx_remove) = []; -- Duke-NUS
clear idx_remove;

% 2. Find Artifact Annotations and Remove Those Points -- Removed by
% Duke-NUS Team
% These are not counted towards the total signal removed
...
% 5. Find Non-'N' Annotations - Removed by Duke-NUS team
...
outliers_combined = idxRRtoBeRemoved(:);% + ann_outliers(:);--Duke-NUS
