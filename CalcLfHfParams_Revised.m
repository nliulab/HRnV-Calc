...
%   - peakvlf/peaklf/peakhf: peak in Hz in VLF, LF and HF --Duke-NUS      
%	REPO:       
...
%%%%%------Duke-NUS---------%%%%
peakvlf = F(find(PSD(Indx_VLF) == max(PSD(Indx_VLF)), 1, 'first')+length(Indx_ULF));
peaklf = F(find(PSD(Indx_LF) == max(PSD(Indx_LF)), 1, 'first')+length(Indx_ULF)+length(Indx_VLF));
peakhf = F(find(PSD(Indx_HF) == max(PSD(Indx_HF)), 1, 'first')+length(Indx_ULF)+length(Indx_VLF)+length(Indx_LF));
%%%%%------Duke-NUS---------%%%%
